...
export class AcademicPaperCategoriesController {
  ...
  @ApiOperation({ summary: 'List all academic paper categories (public, paginated)', description: 'Returns categories that have not been soft-deleted. Use Accept-Language to get translated title and slug. Response is CDN-cacheable (`public, max-age=300, s-maxage=1800`) and varies by `Accept-Language`.' })
  ...
  @Get('trash')
  @Auth('academic-paper-categories:delete')
  @ApiOperation({
    summary: 'List soft-deleted academic paper categories (CMS trash view)',
    description:
      'Paginated list of categories whose `deleted_at` is set. Translation slugs come back with the `__del_<timestamp>` suffix already stripped so the CMS can show the original slug. Requires permission: `academic-paper-categories:delete`.',
  })
  ...
  @Auth('academic-paper-categories:delete')
  @ApiOperation({
    summary: 'Restore a soft-deleted academic paper category',
    description:
      'Sets `deleted_at` back to null and unsuffixes each translation slug. Fails with 409 if any of the original `(lang, slug)` pairs has been claimed by another category in the meantime — rename the conflicting one first. Requires permission: `academic-paper-categories:delete`.',
  })
  ...
  @Get(':id')
  @PublicCache(300, 1800)
  @ApiOperation({ summary: 'Get a single academic paper category (public)', description: 'Returns the category with its translations. Falls back to the default language if no translation exists for the requested language. Response is CDN-cacheable (`public, max-age=300, s-maxage=1800`) and varies by `Accept-Language`.' })
  @ApiParam({ name: 'id', format: 'uuid' })
  ...
  @Post()
  @Auth('academic-paper-categories:create')
  ...
  @Patch(':id')
  @Auth('academic-paper-categories:update')
