...
const baseCategory = { id: 'cat-1', deleted_at: null, academic_paper_category_translations: [] };
...
describe('AcademicPaperCategoriesService', () => {
  ...
  beforeEach(async () => {
      ...
      providers: [
        AcademicPaperCategoriesService,
        {
          ...
          useValue: {
            ...
            academic_paper_category_translations: { upsert: jest.fn().mockResolvedValue({}) },
    ...
    service = module.get<AcademicPaperCategoriesService>(AcademicPaperCategoriesService);
  ...
  describe('findAll', () => {
    it('filters translations by lang when Accept-Language is set', async () => {
      const category = {
        ...baseCategory,
        academic_paper_category_translations: [{ lang: 'ar', title: 'الفقه الإسلامي', slug: 'fiqh-islami' }],
      };
      ...
      expect(prisma.academic_paper_categories.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          include: { academic_paper_category_translations: { where: { lang: 'ar' } } },
        }),
      );
      expect(result.data[0].academic_paper_category_translations[0].lang).toBe('ar');
    });
  ...
  describe('findOne', () => {
    it('returns category with lang-filtered translations', async () => {
      const category = {
        ...baseCategory,
        academic_paper_category_translations: [{ lang: 'en', title: 'Islamic Jurisprudence', slug: 'islamic-jurisprudence' }],
      };
      ...
      expect(prisma.academic_paper_categories.findFirst).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          include: { academic_paper_category_translations: { where: { lang: 'en' } } },
        }),
      );
      expect(result.data.id).toBe('cat-1');
      expect(result.data.academic_paper_category_translations[0].lang).toBe('en');
    });
  ...
  describe('create', () => {
    it('creates category with translations in a transaction', async () => {
      ...
      const result = await service.create(
        { translations: [{ lang: 'ar', title: 'الفقه الإسلامي', slug: 'fiqh-islami' }] },
        'actor-1',
      );

      expect(mockTx.academic_paper_categories.create).toHaveBeenCalled();
      expect(mockTx.academic_paper_category_translations.createMany).toHaveBeenCalled();
      expect(result.data.id).toBe('cat-1');
    });
  });
  ...
  describe('update', () => {
    it('upserts translations for existing category', async () => {
      ...
      expect(prisma.academic_paper_category_translations.upsert).toHaveBeenCalled();
