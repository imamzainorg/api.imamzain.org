...
describe('BooksService', () => {
  ...
  describe('create', () => {
    ...
    it('throws NotFoundException when category not found', async () => {
      ...
      await expect(
        service.create({ category_id: 'bad', cover_image_id: 'm1', translations: [{ lang: 'ar', title: 't', is_default: true }] }, 'u1'),
    ...
    it('throws NotFoundException when cover_image not found', async () => {
      ...
      await expect(
        service.create({ category_id: 'cat-1', cover_image_id: 'bad', translations: [{ lang: 'ar', title: 't', is_default: true }] }, 'u1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ConflictException when ISBN already exists', async () => {
      ...
      await expect(
        service.create(
          { category_id: 'cat-1', cover_image_id: 'media-1', isbn: '978-3-16-148410-0', translations: [{ lang: 'ar', title: 't', is_default: true }] },
          'u1',
        ),
    ...
    it('throws BadRequestException when no default translation', async () => {
      ...
      await expect(
        service.create(
          { category_id: 'cat-1', cover_image_id: 'media-1', translations: [{ lang: 'ar', title: 't', is_default: false }] },
          'u1',
        ),
  ...
  describe('update', () => {
    it('updates book inside a transaction', async () => {
      prisma.books.findFirst.mockResolvedValueOnce(baseBook).mockResolvedValue(null);
    ...
    it('throws NotFoundException when book not found', async () => {
      ...
      await expect(service.update('ghost', {}, 'u1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws ConflictException on duplicate ISBN', async () => {
      ...
      await expect(service.update('book-1', { isbn: '978-0-00-000000-0' }, 'u1')).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('softDelete', () => {
    ...
    it('throws NotFoundException when not found', async () => {
      ...
      await expect(service.softDelete('ghost', 'u1')).rejects.toThrow(NotFoundException);
    });
  });
});
