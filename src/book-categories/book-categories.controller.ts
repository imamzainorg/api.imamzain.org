...
export class BookCategoriesController {
  ...
  @Get('trash')
  @Auth('book-categories:delete')
  @ApiOperation({
    summary: 'List soft-deleted book categories (CMS trash view)',
    description:
      'Paginated list of categories whose `deleted_at` is set. Translation slugs come back with the `__del_<timestamp>` suffix already stripped so the CMS can show the original slug. Requires permission: `book-categories:delete`.',
  })
  @ApiQuery({ name: 'page', required: false, type: Number, example: 1 })
  @ApiQuery({ name: 'limit', required: false, type: Number, example: 20 })
  @ApiOkResponse({ type: BookCategoryListResponseDto, description: 'Paginated list of trashed book categories' })
  @ApiBadRequestResponse({ type: ValidationErrorDto, description: 'Invalid query parameters (page < 1, limit out of 1–100, or non-integer values)' })
  ...
  @Auth('book-categories:delete')
  @ApiOperation({
    summary: 'Restore a soft-deleted book category',
    description:
      'Sets `deleted_at` back to null and unsuffixes each translation slug. Fails with 409 if any of the original `(lang, slug)` pairs has been claimed by another category in the meantime — rename the conflicting one first. Requires permission: `book-categories:delete`.',
  })
  @ApiParam({ name: 'id', format: 'uuid' })
  @ApiOkResponse({ type: BookCategoryMessageResponseDto, description: 'Category restored' })
