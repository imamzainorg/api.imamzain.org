...
const baseCategory = { id: 'cat-1', deleted_at: null, gallery_category_translations: [] };
...
describe('GalleryCategoriesService', () => {
      ...
      providers: [
        GalleryCategoriesService,
        {
          ...
          useValue: {
            ...
            gallery_category_translations: { upsert: jest.fn().mockResolvedValue({}) },
  ...
  describe('findAll', () => {
    it('filters translations by lang when Accept-Language is set', async () => {
      const category = { ...baseCategory, gallery_category_translations: [{ lang: 'ar', title: 'معرض', slug: 'maared' }] };
  ...
  describe('findOne', () => {
    it('returns category with lang-filtered translations', async () => {
      const category = { ...baseCategory, gallery_category_translations: [{ lang: 'en', title: 'Gallery', slug: 'gallery' }] };
  ...
  describe('create', () => {
    it('creates category with translations in a transaction', async () => {
      ...
      expect(mockTx.gallery_category_translations.createMany).toHaveBeenCalled();
      expect(result.data.id).toBe('cat-1');
    });
  });
