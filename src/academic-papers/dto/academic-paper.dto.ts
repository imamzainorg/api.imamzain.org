...
export class CreateAcademicPaperDto {
  @ApiProperty({ format: 'uuid', description: 'ID of an existing academic paper category' })
  @IsUUID()
  category_id: string;
  ...
  @ApiPropertyOptional({ example: 'https://cdn.imamzain.org/papers/paper.pdf', description: 'Direct URL to the PDF file' })
  @IsOptional()
  @IsUrl()
  pdf_url?: string;

  @ApiProperty({ type: [AcademicPaperTranslationDto], description: 'Must include exactly one translation with is_default: true' })
  @IsArray()
...
export class UpdateAcademicPaperDto {
  ...
  @ApiPropertyOptional({ example: 'https://cdn.imamzain.org/papers/updated-paper.pdf' })
  @IsOptional()
  @IsUrl()
  pdf_url?: string;
...
export class AcademicPaperQueryDto extends PaginationDto {
  ...
  @ApiPropertyOptional({ example: 'الصحيفة', description: 'Search across titles' })
  @IsOptional()
  @IsString()
  search?: string;
}
