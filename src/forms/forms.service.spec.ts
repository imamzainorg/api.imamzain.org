...
describe("FormsService", () => {
	...
	describe("updateProxyVisit", () => {
		...
		it("sends WhatsApp when status transitions to COMPLETED", async () => {
			...
			expect(
				whatsappService.sendProxyVisitCompletion,
			).toHaveBeenCalledWith(baseProxyVisit.phone, baseProxyVisit.name)
		})
	...
	describe("softDeleteContact", () => {
		it("sets deleted_at", async () => {
			...
			const result = await service.softDeleteContact(
				"contact-1",
				"admin-1",
			)
