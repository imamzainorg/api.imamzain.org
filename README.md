...
- CMS-managed daily hadiths (`/daily-hadiths`): each hadith optionally carries an editor-set `display_date` for one tied to a specific occasion; `/daily-hadiths/today` (feeds the homepage) serves the hadith scheduled to today or, if none is, a hadith drawn at random from the unscheduled pool and locked in for the rest of the UTC day so every visitor sees the same one; `GET /daily-hadiths` is a pure lookup by exact date, by range, or a plain paginated browse
