...
- CMS-managed daily hadiths (`/daily-hadiths`): each hadith optionally carries an editor-set `display_date` for one tied to a specific occasion; `/daily-hadiths/today` (feeds the homepage) serves the hadith scheduled to today or, if none is, a genuinely random pick from the unscheduled pool, so the answer for a past date never drifts when the pool changes; `GET /daily-hadiths` is a pure lookup by exact date, by range, or a plain paginated browse
